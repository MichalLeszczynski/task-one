...
payload_key_id_output:
	aws cloudformation describe-stacks --query 'Stacks[?StackName==`mlesz-ec2-stack`].Outputs[0][?OutputKey==`KeyPairId`].OutputValue' --output text > key_pair_id.txt # hackish output indexing
...
payload_public_dns_output:
	aws cloudformation describe-stacks --query 'Stacks[?StackName==`mlesz-ec2-stack`].Outputs[1][?OutputKey==`InstanceDnsName`].OutputValue' --output text > public_dns.txt
